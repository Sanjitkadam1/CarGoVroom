...
def detectObjs(): # Detects objects (OpenCV). We were trying to set up an A* but ran out of time...
    ...
    for contour in greenContours:
        ...
        if Cheight > height/10 and (Cwidth*Cheight) > MAX and Cwidth > width/10:
            MAX = Cheight*Cwidth
            if (x > width/2 and (x-Cwidth) > width/2) and (MAX > minArea):
                sideg = "left"
            elif x < width/2: 
                sideg = "right"
            else:
    ...
    if sideg == "left": # If the left is green the right cannot be red
        return sideg
    elif sideg == "right": # If the right is green then center will always be true. (The case of left green is resolved above) 
        return "center"
    else: # No green
        if sider == "right": # If red is right, only right will be true.
            return "right"
        else:
            return "center" # If red is on the left or there are no objects only center can be true.
...
def goto(final): # Compiles a lot of functions above to go to a point of form (x, y, theta) where x and y are relative to the turn and theta is the offset angle of the car from the wall
...
def objResponse(turn, num): # Returns the appropriate response for the detect objects
    if num == 0:
        y = 1100
    elif num == 1:
        y = 1600
...
for _ in range(0, 3):
    for _ in range(0, 4):
        ...
        for i in range(0, 3):
            goto(center, pos[i], 0)
            final = objResponse(detectObjs(), 0)
            goto(final)

# Parrelel parking and reverse was not implemented